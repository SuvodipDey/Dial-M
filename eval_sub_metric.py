...
#----------------------------
"""
Compute sub-metrics on USR datasets:-
...
def evaluate_usr(dataset):
        ...
        for j in range(len(responses)):
            ...
            if(score==-100):
                ...
                if(score>-1):
                    ...
                    overall_score = responses[j]['Understandable']
                    v = sum(overall_score)/len(overall_score)
                    f_under.append(v)
                    
                    overall_score = responses[j]['Natural']
                    v = sum(overall_score)/len(overall_score)
                    f_natural.append(v)
                    
                    overall_score = responses[j]['Maintains Context']
                    v = sum(overall_score)/len(overall_score)
                    f_context.append(v)
                    
                    overall_score = responses[j]['Engaging']
                    v = sum(overall_score)/len(overall_score)
                    f_engage.append(v)
                    
                    overall_score = responses[j]['Uses Knowledge']
                    v = sum(overall_score)/len(overall_score)
                    f_knlg.append(v)
    ...
    corr1, p_val1 = pearsonr(f_gt, f_score)
    corr2, p_val2 = spearmanr(f_gt, f_score)
    logger.write(f"Correlation between Overall and Dial-M score: Pearson = ({round(corr1,4)}, {round(p_val1,4)}), Spearman = ({round(corr2,4)}, {round(p_val2,4)})\n")
    #logger.write("-"*30+"\n")
    
    corr1, p_val1 = pearsonr(f_under, f_score)
    corr2, p_val2 = spearmanr(f_under, f_score)
    logger.write(f"Correlation between Understandable and Dial-M score: Pearson = ({round(corr1,4)}, {round(p_val1,4)}), Spearman = ({round(corr2,4)}, {round(p_val2,4)})\n")
    #logger.write("-"*30+"\n")
    
    corr1, p_val1 = pearsonr(f_natural, f_score)
    corr2, p_val2 = spearmanr(f_natural, f_score)
    logger.write(f"Correlation between Natural and Dial-M score: Pearson = ({round(corr1,4)}, {round(p_val1,4)}), Spearman = ({round(corr2,4)}, {round(p_val2,4)})\n")
    #logger.write("-"*30+"\n")
    
    corr1, p_val1 = pearsonr(f_context, f_score)
    corr2, p_val2 = spearmanr(f_context, f_score)
    logger.write(f"Correlation between Context and Dial-M score: Pearson = ({round(corr1,4)}, {round(p_val1,4)}), Spearman = ({round(corr2,4)}, {round(p_val2,4)})\n")
    #logger.write("-"*30+"\n")
    
    corr1, p_val1 = pearsonr(f_engage, f_score)
    corr2, p_val2 = spearmanr(f_engage, f_score)
    logger.write(f"Correlation between Engaging and Dial-M score: Pearson = ({round(corr1,4)}, {round(p_val1,4)}), Spearman = ({round(corr2,4)}, {round(p_val2,4)})\n")
    #logger.write("-"*30+"\n")
    
    corr1, p_val1 = pearsonr(f_knlg, f_score)
    corr2, p_val2 = spearmanr(f_knlg, f_score)
    logger.write(f"Correlation between Use Knoledge and Dial-M score: Pearson = ({round(corr1,4)}, {round(p_val1,4)}), Spearman = ({round(corr2,4)}, {round(p_val2,4)})\n")
